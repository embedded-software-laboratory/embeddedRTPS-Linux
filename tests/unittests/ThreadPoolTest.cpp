...
protected:
    ...
    void waitForCountAndFailOnTimeout(unsigned int expectedCount, unsigned int maxNumDropped = 0, const std::chrono::milliseconds& time=std::chrono::milliseconds(10)){
        std::unique_lock<std::mutex> lock(mut);
...
// Currently failes because of busy waiting in reader thread
TEST_F(ThreadPool_WithReadCallback, DISABLED_ProcessesAlmostAllPackets){
