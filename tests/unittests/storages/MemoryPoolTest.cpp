...
TEST_F(JustCreatedMemoryPool, loopWithPostIncrement_showsZeroElements){
    uint8_t count = 0;
...
TEST_F(MemoryPoolWithThreeElements, findCorrectElement){
    const uint8_t data = dataTwo;

    auto callback=[data](const uint8_t& value){return value == data;};
    auto thunk=[](void* arg, const uint8_t& value){return (*static_cast<decltype(callback)*>(arg))(value);};

    auto* result = pool.find(thunk, &callback);
