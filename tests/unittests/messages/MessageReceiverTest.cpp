...
protected:
    ...
    BufferMock validDataMsgBufferMockUE;
    rtps::GuidPrefix_t somePrefix = {1,2,3,4};
    BufferMock somePayload;

    ReaderMock prefixReaderMock{{somePrefix, rtps::ENTITYID_SPDP_BUILTIN_PARTICIPANT_READER}};
    ...
    void SetUp() override{
        ...
        rtps::MessageFactory::addHeader(validDataMsgBufferMockUE, somePrefix);
        rtps::MessageFactory::addSubMessageData(validDataMsgBufferMockUE, somePayload, false, rtps::SequenceNumber_t{0,1},
                                                rtps::ENTITYID_SPDP_BUILTIN_PARTICIPANT_WRITER,
                                                rtps::ENTITYID_UNKNOWN);
    };
};
...
TEST_F(AMessageReceiverReceivedDataSubmessage, processMessage_DataSMWithUnknownEntitiIdAddsCacheChangeToCorrectReader){
    part.addReader(&anotherReaderMock);
    part.addReader(&prefixReaderMock);

    auto data = validDataMsgBufferMock.buffer.data();
    auto size = (rtps::DataSize_t) validDataMsgBufferMock.buffer.size();

    EXPECT_CALL(prefixReaderMock, newChange(_)).Times(1);
    EXPECT_CALL(anotherReaderMock, newChange(_)).Times(0);
