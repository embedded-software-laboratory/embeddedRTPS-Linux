...
void MeasurementUnit::prepareRTPS(){
    ...
    mp_dataWriter = m_domain.createWriter(*part, "LatencyTest_PUB2SUB", "LatencyType", false);
    mp_dataReader = m_domain.createReader(*part, "LatencyTest_SUB2PUB", "LatencyType", false);
...
void MeasurementUnit::measurementJumppad(void* callee, const rtps::ReaderCacheChange& cacheChange){
    auto measurementUnit = static_cast<MeasurementUnit*>(callee);
    measurementUnit->measurementCallback(cacheChange);
}

void MeasurementUnit::measurementCallback(const rtps::ReaderCacheChange& /*cacheChange*/){
